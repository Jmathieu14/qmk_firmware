...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
	...
	[DEFAULT] = LAYOUT( \
		LT(NUM,KC_ESC)  , KC_Q          , KC_W          , KC_F          , KC_P          , KC_B                         , KC_J          , KC_L          , KC_U          , KC_Y          , KC_SCLN       , KC_DEL         , KC_BSPC, \
		LT(MEDIA,KC_TAB), KC_A          , KC_S          , KC_R          , KC_T          , KC_G                         , KC_M          , KC_N          , KC_E          , KC_I          , KC_O          , TD(NTR_BCKSLSH), \
		KC_LCTL         , KC_X          , KC_C          , KC_D          , KC_V          , KC_Z   , LT(SYM_FUNC,KC_QUOT), KC_K          , KC_H          , KC_COMM       , KC_DOT        , KC_SLSH        , MO(NUM), \
		KC_LSFT         , KC_LGUI       , KC_LALT       , LSFT_T(KC_SPC), KC_SPC        , MO(NAV), KC_RALT             , KC_RCTL       ),

	[GAME] = LAYOUT( \
		KC_TAB , KC_Q   , KC_W   , KC_E   , TD(R_ONE)  , TD(T_FOUR), TD(Y_SEVEN), TD(U_ZERO) , KC_I  , KC_O   , KC_P   , KC_TRNS, KC_TRNS, \
		KC_SPC , KC_A   , KC_S   , KC_D   , TD(F_TWO)  , TD(G_FIVE), TD(H_EIGHT), KC_J       , KC_K  , KC_L   , KC_SCLN, KC_TRNS, \
		KC_LCTL, KC_Z   , KC_X   , KC_C   , TD(V_THREE), TD(B_SIX) , KC_TRNS    , TD(N_NINE) , KC_M  , KC_TRNS, KC_TRNS, KC_TRNS, KC_TRNS, \
...
void matrix_init_user(void) {
  ...
  setPinOutput(B1);
  writePinLow(B1);
  // set NumLock LED to output and low8
  setPinOutput(B2);
  writePinLow(B2);
